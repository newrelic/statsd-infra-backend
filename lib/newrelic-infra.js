...
var collectMetrics = function nriCollectMetrics(rawMetrics, rules) {
  ...
  var evalRule = function evalRule(rule, metricName, value, tags) {
    ...
    // Get the tags for the metric
    try {
      tags = tags[metricName];
    } catch(e) {
    ...
    if (found) {
      if (validateKeyWithSchema(metricName, rule.metricSchema)) {
        ...
        // Add the tags as attributes to the event
  ...
  Object.keys(counters).forEach(function(metricName) {
    rules.forEach(function(rule) {
      if (evalRule(rule, metricName, counters[metricName]), counters_tags) {
        evalRule(rule, metricName + 'PerSecond', counterRates[metricName], counter_rates_tags);
        matchedKeys++;
      }
    });
  });
  ...
  Object.keys(timerData).forEach(function(metricName) {
    rules.forEach(function(rule) {
      Object.keys(timerData[metricName]).forEach(function(timerKey) {
        evalRule(rule, metricName + '.' + timerKey, timerData[metricName][timerKey], timer_data_tags);
      });
    });
  });
  ...
  Object.keys(gauges).forEach(function(metricName) {
    rules.forEach(function(rule) {
      evalRule(rule, metricName, gauges[metricName], gauges_tags);
    });
  });

  Object.keys(sets).forEach(function(metricName) {
    rules.forEach(function(rule) {
      evalRule(rule, metricName + '.count', sets[metricName].size(), sets_tags);
    });
  });
